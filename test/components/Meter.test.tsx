...
describe('Meter', () => {
  it('renders correctly', () => {
    const component = renderer.create(<Meter {...exampleProps} />);
  ...
  it('computes basisSize with custom min and max', () => {
    const meter = shallow(<Meter value={150} min={100} max={200} />);
    expect(meter.find('.filling').prop('basis')).toBe('50%');
  });

  it('computes basisSize for custom negative min', () => {
    const meter = shallow(<Meter value={0} min={-100} max={100} />);
    expect(meter.find('.filling').prop('basis')).toBe('50%');
  });

  it('computes background color for filling', () => {
    const ranges = [{ startValue: 50, endValue: 80, fillingColor: 'yellow' }];
    const meter = shallow(<Meter value={60} ranges={ranges} />);
    expect(meter.find('.filling').prop('style').background).toBe(ranges[0].fillingColor);
  });

  it("defaults to the base color as background color if there's no matching range", () => {
    const ranges = [{ startValue: 50, endValue: 80, fillingColor: 'yellow' }];
    const meter = shallow(<Meter value={20} ranges={ranges} baseFillingColor="#ccc" />);
  ...
  it("doesn't define a background color if there's no matching range and no default is given", () => {
    const ranges = [{ startValue: 50, endValue: 80, fillingColor: 'yellow' }];
    const meter = shallow(<Meter value={20} ranges={ranges} />);
  ...
  it("defaults to base color as bar background color should if there's no matching range", () => {
    const ranges = [{ startValue: 50, endValue: 80, fillingColor: 'yellow' }];
    const meter = shallow(<Meter value={20} ranges={ranges} baseBackgroundColor="#ccc" />);
  ...
  it('computes colors respecting base colors', () => {
    ...
    const meter1 = shallow(<Meter value={30} ranges={ranges} {...baseColors} />);
    ...
    const meter2 = shallow(<Meter value={60} ranges={ranges} {...baseColors} />);
