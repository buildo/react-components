...
export interface UpdateProps<T, K extends keyof T> {
  col: React.ReactElement<ColumnProps<T, K>>,
  index: number,
  colGroup?: React.ReactElement<ColumnGroupProps<T>>
};
export type UpdateHandler<T, K extends keyof T> = (c: UpdateProps<T, K>) => JSX.Element;
...
export const updateColumns = curry(<T, K extends keyof T>(children: TabloChildren<T, K>, update: UpdateHandler<T, K>): JSX.Element[] => {
