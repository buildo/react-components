...
type TabloChildren<T, K extends string> = React.ReactElement<Column.Props<T, K>>[] | React.ReactElement<ColumnGroupProps<T, K>>[];
...
export const updateColumns = curry(<T, K extends string>(children: TabloChildren<T, K>, update: UpdateColumnsHandler<T, K>): JSX.Element[] => {
  const updateIfColumn = (colGroup?: React.ReactElement<ColumnGroupProps<T, K>>) => (col: React.ReactElement<Column.Props<T, K>>, index: number) => col.type === Column ? update({ col , index, colGroup }) : col;
  const chArray = Children.toArray(children) as React.ReactElement<Column.Props<T, K> | ColumnGroupProps<T, K>>[];
  const thereAreGroups = chArray.filter(ch => ch.type === ColumnGroup).length > 0;
